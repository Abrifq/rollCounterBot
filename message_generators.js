...
function beforeRollMessageConstructor({ userID, arguments: { target, diceSides } }) {
    if (donators.includes(userID)) { return `<@${userID}> aşkım bana şans dile, atıyorum ${diceSides} yüzlü zarımı, senin için ${target} atacağım bebeğim!${target > 1000 ? " Ama biraz büyük sayı, beklemen gerekebilir aşkoom. :kiss:" : ""}`; }
    return `<@${userID}>, bana şans dile, ${target} sayısı için ${diceSides} yüzlü zarımı atmaya başladım.${target > 1000 ? " Büyük sayı yani." : ""}`;
}
